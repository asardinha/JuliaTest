...
function createOutputcsvFromTracer(tracer, players, P, C, oneB, twoB, threeB,SS,OF,path_to_output)
    ...
    SS_index = 5
    OF_index = 6
    GrowingLineupMatrix = "P,C1B,2B,3B,SS,OF,OF,OF,UTIL\n"
    for j = 1:num_lineups
        ...
        for i =1:num_players
            if tracer[i,j] == 1
                ...
                elseif C[i]==1
                    ...
                    elseif lineup[9] ==""
                        lineup[9] = string(players[i,:Player_Name])
                    else
                        println(string("error in lineup",j,"player",i))
                    end
                elseif oneB[i]==1
...
function create_lineups(num_lineups, num_overlap, stack_size,formulation, path_pitchers,path_hitters,  path_to_output)
    ...
    for j=1:num_stacks
        if players[1,:Batting_Order_Confirmed_] == j
            stack_ind = circshift(collect(1:9),stack_order-j)[1:stack_order[4]];  #index of the stacks this batting order belongs to.
...
function lineup_points_proj(path_lineups,path_hitters,path_pitchers,path_output)
    ...
    num_players = size(players)[1]
    GrowingLineupMatrix = "P,C1B,2B,3B,SS,OF,OF,OF,Proj_pts\n"
    for i = 1:num_lineups
        lineup_pts = 0
        for j = 1:num_spots
            name = lineups[i,j]
