...
function baseball_formulation(players, old_lineups, num_overlap,stack_size, P,B1,B2,B3,C,SS,OF, players_teams, players_opp, players_games,players_stacks)
    ...
    @addConstraint(m, sum{players_lineup[i], i=1:num_players} == 9)

    #SALARY: Financial Constraint - salary less than $50,000
    @addConstraint(m, sum{players[i,:Salary]*players_lineup[i], i=1:num_players}<= 35000)

    #POSITION
    #  2 P constraint
    @addConstraint(m, sum{P[i]*players_lineup[i], i=1:num_players}==2)
    # one B1 constraint
    @addConstraint(m, sum{B1[i]*players_lineup[i], i=1:num_players}<=2)
	@addConstraint(m,1 <= sum{B1[i]*players_lineup[i], i=1:num_players})

    # one B2 constraint
    @addConstraint(m, sum{B2[i]*players_lineup[i], i=1:num_players}<=2)
	@addConstraint(m,1 <= sum{B2[i]*players_lineup[i], i=1:num_players})
    # one B3 constraint
    @addConstraint(m, sum{B3[i]*players_lineup[i], i=1:num_players}<=2)
	@addConstraint(m,1 <= sum{B3[i]*players_lineup[i], i=1:num_players})
    # one C constraint
    @addConstraint(m, sum{C[i]*players_lineup[i], i=1:num_players}<=1)
	@addConstraint(m,0 <= sum{C[i]*players_lineup[i], i=1:num_players})
    # one SS constraint
    @addConstraint(m, sum{SS[i]*players_lineup[i], i=1:num_players}<=2)
	@addConstraint(m,1 <= sum{SS[i]*players_lineup[i], i=1:num_players})
    # 3 OF constraint
    @addConstraint(m, sum{OF[i]*players_lineup[i], i=1:num_players}<=4)
	@addConstraint(m,3 <= sum{OF[i]*players_lineup[i], i=1:num_players})
